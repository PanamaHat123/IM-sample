export default class ByteBuffer {
    ...
    Type_String:number = 6;
    Type_VString:number = 7;
    Type_Int64:number = 8;
    Type_Float:number = 9;
    ...
    blength () {
        ...
        /**
         * The first 4 bytes of a variable-length string indicate the string length
         **/
     string(val, index) {
        if (arguments.length == 0) {
    ...
    /**
     * Read a fixed-length string when val is null (length len needs to be specified)
     **/
    ...
    /**
     * Pack as binary, with 4 bytes in front to indicate the package length
     **/
    packWithHead() {
    ...
    /**
         * Package as binary
         * @param ifHead Whether to add 4 bytes before indicates the packet length
         **/
     pack(ifHead) {
        ...
        for (var i = 0; i < this._list.length; i++) {
            switch (this._list[i].t) {
                ...
                case this.Type_VString:
                    this.utf8Write(this._org_buf, offset, this._list[i].d);
                    var vlen = this.utf8Length(this._list[i].d);//Actual string length
                    //Complete \0
                ...
                case this.Type_ByteArray:
                    ...
                    for (var j = offset; j < offset + this._list[i].l; j++) {
                        ...
                        } else {// If not enough, complete with 0x00
                            this._org_buf.setUint8(j, 0);
                        }
                        indx++
                    }
